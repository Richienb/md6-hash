...
function preHash(data = "", size = 256, key = "", levels = 64) {
    data = bytes(data)
    key = bytes(key)
...
function md6Hash(data, size, key, levels) {
    const hash = preHash(data, size, key, levels)

    return hash.map((v) => String.fromCharCode(v)).join("")
}

md6Hash.hex = (data, size, key, levels) => {
    const hash = preHash(data, size, key, levels)
